...
fun Long.readableFileSize() : String {
    ...
    val digitGroups = (Math.log10(this.toDouble())/Math.log10(1024.0));
    val scaledSize = this / Math.pow(1024.0, digitGroups);
    val format = if (scaledSize < 100) "#,##0.##" else "#,##0.#"

    return DecimalFormat(format).format(scaledSize) + " " + units[digitGroups.toInt()]
}
